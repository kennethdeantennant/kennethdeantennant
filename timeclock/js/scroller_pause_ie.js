...
divtev4=")\"";
divtexts = " style=\"position:absolute;visibility:hidden;width:"+tmpv+"; background:#FFFFFF; COLOR: #000000; left:0; top:0; FONT-FAMILY: MS Sans Serif,arial,helvetica; FONT-SIZE: 8pt; FONT-STYLE: normal; FONT-WEIGHT: normal; TEXT-DECORATION: none; margin:0px;padding:0px;overflow-x:hidden; LINE-HEIGHT: 12pt; text-align:left;padding:0px; cursor:'default';\">";
ie6span= " style=\"position:relative;background: #FFFFFF; COLOR: #414A76; width:"+tmpv+"; FONT-FAMILY: verdana,arial,helvetica; FONT-SIZE: 9pt; FONT-STYLE: normal; FONT-WEIGHT: bold; TEXT-DECORATION: none; LINE-HEIGHT: 12pt; text-align:left;padding:0px;\"";
...
function dotrans()
{
	if(inoout==false)
	{
		...
		{
			objd=eval("d"+i);
			objd.style.pixelTop=uzuntop+(i*winHeight);
			if((uzuntop+(i*winHeight))==4){dahayok=true;}
		}
		if(uzuntop<(-1*(mc-1)*winHeight))
		{
			objd=eval("d"+0);
			objd.style.pixelTop=uzuntop+(mc*winHeight);
			if((uzuntop+(i*winHeight))==4){dahayok=true;}		
		}
...
function initte2()
{
	...
	for(i=0;i<mc;i++)
	{
		...
		objd.style.pixelTop=toplay+(winHeight*i);
		objd.style.pixelLeft=leftMargin;
